...
function setup() {
    let header = select('header'); // Выбираем элемент header
    let headerHeight = header.size().height; // Получаем высоту header
    let headerWidth = header.size().width; // Получаем высоту header
    let cnv = createCanvas(headerWidth, headerHeight);
    ...
    strokeWeight(1);
    numRows = Math.ceil(headerHeight / CELL_SIZE); // number of rows in the grid
    numCols = Math.ceil(headerWidth / CELL_SIZE); // number of columns in the grid
}
...
function windowResized() {
    headerHeight = select('header').size().height; // Обновляем высоту header
    headerWidth = select('header').size().width; // Обновляем ширину header
    resizeCanvas(headerWidth, headerHeight);
    numRows = Math.ceil(headerHeight / CELL_SIZE); // количество строк в сетке
    numCols = Math.ceil(headerWidth / CELL_SIZE); // количество столбцов в сетке
}